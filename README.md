...
1. Создать файл .env в корне проекта с необходимыми переменными окружения (см. configs/config.prod.yaml для примера):
2. Запустить все сервисы через Docker:
   ```bash
   scripts\run-local.bat
   ```
3. Создать администратора и запустить эмулятор устройств:
   ```bash
...
```bash
scripts\stop-local.bat          # остановить, сохранить данные
scripts\stop-local.bat --full   # остановить и сбросить БД
```
...
7. **Эмуляция для тестов**: Для разработки без реальных устройств используйте 
   официальные симуляторы (Health Services Simulator) или генерацию синтетических 
   данных с реалистичными паттернами.
