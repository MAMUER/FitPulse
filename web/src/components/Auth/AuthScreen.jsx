...
export default function AuthScreen() {
  ...
  const [passwordChecks, setPasswordChecks] = useState({
    length: false, upper: false, lower: false, digit: false,
  });
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setSuccessMessage(data.message || 'Регистрация успешна. Подтвердите email.');
      setMode('verify');
    } catch (err) {
      setGeneralError(err.message);
    } finally {
      ...
      <div className="auth-container">
        ...
        {mode === 'login' && (
          <form className="auth-form" onSubmit={handleLogin} novalidate>
            ...
            <p className="auth-switch">
              Нет аккаунта? <a href="#" onClick={(e) => { e.preventDefault(); setMode('register'); }}>Создать</a>
            </p>
          </form>
        )}
        ...
        {mode === 'register' && (
          <form className="auth-form" onSubmit={handleRegister} novalidate>
            ...
            <div className="field">
              ...
              />
              <div className="field-error">{errors.password || ''}</div>
              <div className={`password-hint ${formData.password ? '' : 'hidden'}`}>
            ...
            <div className="auth-error hidden">{generalError}</div>
            <button type="submit" className="btn-primary" disabled={submitting || !!Object.values(passwordChecks).find(v => !v)}>
              {submitting ? 'Создание...' : 'Создать аккаунт'}
            </button>
            <p className="auth-switch">
              Уже есть? <a href="#" onClick={(e) => { e.preventDefault(); setMode('login'); }}>Войти</a>
            </p>
          </form>
        )}
        ...
        {mode === 'login2fa' && (
          <form className="auth-form" onSubmit={handleLogin2FA} novalidate>
            ...
            <p className="auth-switch">
              <a href="#" onClick={(e) => { e.preventDefault(); setMode('login'); setTwoFATempToken(null); }}>
                ← Вернуться ко входу
              </a>
            </p>
          </form>
        ...
        {mode === 'verify' && (
          <div className="auth-form verify-form">
            ...
            <p className="verify-text">
              Мы отправили письмо на <strong>{formData.email || 'ваш email'}</strong>
            </p>
            <p className="verify-text">Перейдите по ссылке из письма, чтобы подтвердить email и войти.</p>
