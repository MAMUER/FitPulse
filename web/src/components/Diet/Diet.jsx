...
const MEAL_TEMPLATES = {
  balanced: {
    ...
    breakfast: [
      { name: 'Овсяная каша с бананом', kcal: 350, protein: 12, carbs: 56, fat: 7 },
      { name: 'Гречневая каша с яйцом', kcal: 380, protein: 18, carbs: 42, fat: 12 },
      { name: 'Тосты с авокадо и яйцом', kcal: 400, protein: 16, carbs: 30, fat: 24 },
    ],
    snack1: [
    ...
    lunch: [
      { name: 'Куриная грудка с рисом', kcal: 450, protein: 35, carbs: 50, fat: 10 },
      { name: 'Суп с овощами и курицей', kcal: 380, protein: 25, carbs: 30, fat: 15 },
      { name: 'Паста с томатным соусом', kcal: 420, protein: 18, carbs: 55, fat: 12 },
    ],
    snack2: [
      { name: 'Протеиновый коктейль', kcal: 200, protein: 25, carbs: 15, fat: 4 },
      { name: 'Яблоко с арахисовым маслом', kcal: 220, protein: 6, carbs: 25, fat: 12 },
      { name: 'Батончики из цельнозерна', kcal: 180, protein: 8, carbs: 25, fat: 7 },
    ],
    dinner: [
      { name: 'Запечённый лосось с овощами', kcal: 450, protein: 35, carbs: 20, fat: 25 },
  ...
  high_protein: {
    name: 'Высокобелковое',
    breakfast: [
      { name: 'Омлет с сыром и ветчиной', kcal: 400, protein: 30, carbs: 4, fat: 28 },
      { name: 'Творожная запеканка', kcal: 380, protein: 28, carbs: 30, fat: 14 },
      { name: 'Сырники с гречкой', kcal: 420, protein: 25, carbs: 40, fat: 15 },
    ],
    snack1: [
      { name: 'Протеиновый коктейль', kcal: 200, protein: 30, carbs: 10, fat: 3 },
    ...
    lunch: [
      ...
      { name: 'Курица гриль с салатом', kcal: 450, protein: 40, carbs: 20, fat: 22 },
      { name: 'Тунец с спаржей', kcal: 400, protein: 38, carbs: 10, fat: 24 },
    ],
    snack2: [
      { name: 'Протеиновые крекеры', kcal: 220, protein: 20, carbs: 20, fat: 6 },
      { name: 'Творожная паста', kcal: 200, protein: 18, carbs: 8, fat: 10 },
      { name: 'Изолят сывороточного протеина', kcal: 180, protein: 35, carbs: 5, fat: 2 },
    ],
    dinner: [
      { name: 'Индейка с брюссельской капустой', kcal: 450, protein: 42, carbs: 18, fat: 22 },
      { name: 'Суп из чечевицы с курицей', kcal: 400, protein: 32, carbs: 35, fat: 10 },
  ...
  weight_loss: {
    ...
    breakfast: [
      ...
      { name: 'Овсяная каша на воде', kcal: 220, protein: 8, carbs: 38, fat: 4 },
    ...
    lunch: [
      { name: 'Салат с куриной грудкой', kcal: 320, protein: 30, carbs: 15, fat: 14 },
      { name: 'Суп овощной', kcal: 180, protein: 8, carbs: 20, fat: 4 },
      { name: 'Запечённая индейка с брокколи', kcal: 350, protein: 32, carbs: 12, fat: 16 },
    ],
    snack2: [
      { name: 'Греческий йогурт', kcal: 120, protein: 12, carbs: 8, fat: 4 },
      { name: 'Протеиновый батончик', kcal: 180, protein: 15, carbs: 20, fat: 6 },
    ...
    dinner: [
      ...
      { name: 'Крем-суп из кабачков', kcal: 220, protein: 10, carbs: 15, fat: 12 },
    ],
  },
};
...
export default function Diet() {
  ...
  const nutrition = useMemo(() => {
    ...
    const activityMultipliers = { beginner: 1.375, intermediate: 1.55, advanced: 1.725 };
    const tdee = bmr * (activityMultipliers[fitness] || 1.375);
    const goalAdjustments = { weight_loss: -400, muscle_gain: 300, endurance: 100 };
    const calories = Math.max(1200, Math.round(tdee + (goalAdjustments[goal] || 0)));
    ...
    return { bmr: Math.round(bmr), tdee: Math.round(tdee), calories, proteinG, fatG, carbsG, fitness, goal };
  }, [profile]);
  ...
  useEffect(() => {
    if (!nutrition) return;
    const selectedTemplate = MEAL_TEMPLATES[template] || MEAL_TEMPLATES.balanced;
    ...
    const generated = selectedMealKeys.map((key, idx) => {
      ...
      const meal = options[Math.floor(Math.random() * options.length)] || { name: '—', kcal: 0, protein: 0, carbs: 0, fat: 0 };
      const timeMinutes = mealCount > 1 ? startMinutes + idx * step : startMinutes;
  ...
  if (!nutrition) return <div className="view active">Ошибка загрузки профиля</div>;

  return (
    <div className="view active">
      <div className="diet-summary">
        <div className="diet-calories">{nutrition.calories}</div>
        <div className="diet-label">ккал в день · BMR {nutrition.bmr} · {nutrition.fitness === 'beginner' ? 'Начинающий' : nutrition.fitness === 'intermediate' ? 'Средний' : 'Продвинутый'}</div>
      ...
      <div className="diet-controls">
        ...
        <div className="form-group">
          ...
          <select value={mealCount} onChange={e => setMealCount(Number(e.target.value))}>
            {[3, 4, 5, 6].map(n => <option key={n} value={n}>{n}</option>)}
          </select>
        </div>
        <div className="form-group">
          <label>Время первого приёма</label>
          <input type="time" value={firstMealTime} onChange={e => setFirstMealTime(e.target.value)} />
        </div>
        <div className="form-group">
          <label>Аллергии (через запятую)</label>
          <input type="text" value={allergies} onChange={e => setAllergies(e.target.value)} placeholder="Например: орехи, лактоза" />
        </div>
        <div className="form-group">
          <label>Нелюбимые продукты (через запятую)</label>
          <input type="text" value={dislikes} onChange={e => setDislikes(e.target.value)} placeholder="Например: брокколи, рыба" />
        </div>
      </div>
            ...
            meals.map((meal, idx) => (
              <div key={idx} className="meal-card">
                ...
                <div className="meal-details">
                  {meal.kcal} ккал · Б:{meal.protein}г · Ж:{meal.fat}г · У:{meal.carbs}г
                </div>
              </div>
            ))
          )}
