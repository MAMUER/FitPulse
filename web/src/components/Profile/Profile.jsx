...
export default function Profile() {
  ...
  const [form, setForm] = useState({
    ...
    fitness: '', nutrition: '', allergies: '', contraindications: '',
    goal: '',
  });
  ...
  return (
    <div className="view active profile-view">
      <form className="profile-form" onSubmit={handleSubmit}>
        ...
        <div className="form-section">
          ...
          {form.height && form.weight && (() => {
            const bmi = calculateBMI(parseFloat(form.height), parseFloat(form.weight));
            if (bmi) {
              return (
        ...
        <div className="form-section">
          ...
          <div className="form-group">
            ...
            <select value={form.nutrition} onChange={e => setField('nutrition', e.target.value)}>
              <option value="">—</option>
        ...
        <div className="form-section">
          ...
          <div className="form-group">
            <label>Аллергии или непереносимость продуктов (через запятую)</label>
            <input
              type="text"
        ...
        <div className="form-section">
          ...
          <div className="goals-grid">
            {['weight_loss', 'muscle_gain', 'endurance', 'flexibility'].map(goal => (
              <label key={goal} className={`goal-chip ${form.goal === goal ? 'selected' : ''}`}>
                <input
                  type="radio"
                  name="goal"
                  value={goal}
      ...
      <div className="form-section" style={{ marginTop: 24 }}>
        <h3>Безопасность</h3>
        <button type="button" className="btn-secondary" onClick={() => setShowPasswordModal(true)} style={{ marginBottom: 12 }}>
          Сменить пароль
        </button>
        <button type="button" className="btn-secondary" onClick={() => setShowEmailModal(true)} style={{ marginBottom: 12 }}>
          Сменить почту
        </button>
      ...
      <div className="form-section danger-zone" style={{ marginTop: 24 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12 }}>
          Удаление профиля необратимо. Все ваши данные, тренировки и достижения будут удалены.
        </p>
      ...
      {showDeleteModal && <DeleteProfileModal onClose={() => setShowDeleteModal(false)} />}
    </div>
  );
}
