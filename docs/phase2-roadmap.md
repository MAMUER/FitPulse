...
Single Redis (теперь Valkey) используется для кэширования и сессий. Redis 7.4+ перешёл на RSALv2 лицензии, рекомендуется миграция на Valkey 9 (open-source форк).
...
- Совместимость с redis-cli и go-redis клиентом
