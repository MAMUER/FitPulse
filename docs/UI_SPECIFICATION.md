...
- `confirmPassword`
- Кнопки: `cancelChangePassword`, submit → `PUT /profile/security/password` (предполагаемый эндпоинт).
...
| Сменить пароль | PUT | `/profile/security/password` (предполагаемый) |
| Сменить email | PUT | `/profile/security/email` (предполагаемый) |
...
| 4 | JWT в `localStorage` | Хранится под ключом `authToken`. При logout — удаляется. |
