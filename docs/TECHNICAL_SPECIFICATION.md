...
**3.2.** Эксплуатация: мобильное веб-приложение (6 вкладок: Обзор, Профиль, Тренировки, Устройства, Диета + 1 скрытая вкладка администратора).

**3.3.** Детали: проверка биометрических данных, многоканальная интеграция (ROOK/Terra/Health Connect/HealthKit), гибридный подход для противопоказаний, криптозащита резервных копий.
...
- аутентификацию по JWT + Refresh Token с rotation и reuse detection. Подробности: [API Reference → Аутентификация](docs/API.md#аутентификация);
...
- сервер: Linux x86_64, 2 ГБ ОЗУ, 30 ГБ диска;
...
|Внедрение|Kubernetes манифесты (Kustomize), CI/CD Pipeline|—|
