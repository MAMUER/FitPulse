...
4. **Устойчивость**: восстановление после сбоя <5 минут вручную (manual rollback). Chaos Engineering (pod kills, network partitions) запланирован на Phase 2. MTTR трекается в Grafana.

5. **Безопасность**: 0 критических уязвимостей после пентеста. Квартальные внешние пентесты и ежемесячные внутренние сканирования запланированы на Phase 2. Текущие меры: автоматические SAST/DAST (Trivy, govulncheck, gosec) в CI/CD. SLA remediaton (best effort, без юридических гарантий: critical — 1–3 рабочих дня, high — 3–7 рабочих дней).
...
- реализован мониторинг и алертинг для доступности и производительности (Prometheus probes, гистограммы latency);
- нагрузочное тестирование через k6 запланировано на Phase 2;
- Chaos Engineering и внешние пентесты запланированы на Phase 2;
- поддерживаются стандарты документации и процессы review.
