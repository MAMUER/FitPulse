...
if WANDB_ENABLED:
    ...
    wandb.init(
        ...
        name=f"diffusion_v4_{datetime.now().strftime('%Y%m%d_%H%M')}",
    )
else:
    ...
    def sample(self, condition=None, num_steps=1000):
        ...
        for i in reversed(range(num_steps)):
            ...
            mean = (
                ...
                + (1 - alpha_bar_prev) / torch.sqrt(1 - alpha_bar_t) * noise_pred
            )
...
def load_real_data():
    ...
    if not TRAINING_DATA_PATH.exists():
        if FALLBACK_TRAINING_DATA_PATH.exists():
            print(
                f"Warning: {TRAINING_DATA_PATH} not found, "
                f"using {FALLBACK_TRAINING_DATA_PATH} (no conditions)"
            )
    ...
    if "condition_vector" in df.columns:
        conditions = np.array(
            [ast.literal_eval(x) for x in df["condition_vector"]], dtype=np.float32
        )
...
def train_and_save():
    ...
    print(
        f"Best val_loss: {checkpoint_callback.best_model_score:.4f}"
