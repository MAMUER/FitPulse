...
| **user-service** `internal/email/email.go:140-148` | S5332: clear-text SMTP в ветке `else` (когда `SMTP_TLS=false`) | Принятый риск: по умолчанию `SMTP_TLS=true`; non-TLS путь (`smtp.SendMail`) доступен только при явном указании `SMTP_TLS=false` и предназначен для локальной разработки с MailHog (порт 1025, локальная сеть Docker). В production `SMTP_TLS=true` обязателен. Тесты переписаны на моки (`sendWithTLSImpl`, `smtpSendMailFn`), не зависят от MailHog | В production обязательно включать `SMTP_TLS=true`; для локальной разработки с MailHog явно устанавливать `SMTP_TLS=false` |
