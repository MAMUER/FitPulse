...
### CodeQL — принятые исключения (log-injection)

CodeQL `go/log-injection` флагает логирование пользовательских полей даже после санитизации через `sanitize.LogString()`. Это **ложноположительное**, потому что `LogString` удаляет все ASCII управляющие символы (0x00-0x1F, 0x7F), предотвращая инъекцию новых строк в логи.

| Правило | Файл | Обоснование |
| ------- | ------ | ----------- |
| `go/log-injection` | `internal/webhook/handler.go:111,147,148,216,227,237` | Поля `user_id`, `source`, `payload.UserID` санитизированы через `sanitize.LogString()` перед записью в лог. |
| `go/log-injection` | `internal/webhook/storage.go:114,115` | Поля `user_id`, `source` санитизированы через `sanitize.LogString()` перед записью в лог. |
